...
HEDEF_Z = float(norm.ppf(HEDEF_HIZMET_DUZEYI))
...
def hizmet_duzeyi(r, mu_L, sigma_L):
    return float(norm.cdf((r - mu_L) / sigma_L)) if sigma_L > 0 else 1.0


def min_hizmet_r(mu_L, sigma_L, hedef=HEDEF_HIZMET_DUZEYI):
    z = float(norm.ppf(hedef))
    return max(0, int(np.ceil(mu_L + z * sigma_L)))
...
def minmax_politika(mu, sigma, LT_ay, h, p, S, z_hizmet=HEDEF_Z):
    """
    ...
    r   = max(min_hizmet_r(mu_L, sigma_L), int(round(mu_L + z_hizmet * sigma_L)))
    SS  = max(0, int(round(r - mu_L)))
    ...
    return {
        ...
        "sim_SI":     komp["SI"],
        "sim_SK":     komp["SK"],
        "sim_HZ":     hizmet_duzeyi(r, mu_L, sigma_L),
        "hedef_HZ":    HEDEF_HIZMET_DUZEYI,
...
def grid_search(mu, sigma, LT_ay, h, p, S, Q_ref, adim=15):
    mu_L, sigma_L = lt_param(mu, sigma, LT_ay)
    Q_ref = max(Q_ref, 1.0)
    r_min = min_hizmet_r(mu_L, sigma_L)
    Q_grid = np.unique(np.linspace(max(1., Q_ref * 0.3), Q_ref * 4, adim).astype(int))
    r_grid = np.unique(np.linspace(r_min, max(r_min + 1., mu_L + 3 * sigma_L), adim).astype(int))
    best_tc, best_Q, best_r = np.inf, Q_ref, r_min
    for Qv in Q_grid:
        for rv in r_grid:
...
def optuna_optimize(mu, sigma, LT_ay, h, p, S, best_Qg, best_rg,
    ...
    r_lb = max(float(r_min), best_rg - sigma_L)
    r_ub = max(best_rg + sigma_L, float(r_min + 1))
    ...
    opt_r  = max(r_min, int(round(s.best_params["r"])))
    opt_SS = max(0, int(round(opt_r - mu_L)))
    hiz    = hizmet_duzeyi(opt_r, mu_L, sigma_L)
...
def hizmet_duzeyi_garanti(Q, r, mu, sigma, LT_ay, h, p, S,
                          talep_serisi, n_rep=20, hedef=HEDEF_HIZMET_DUZEYI):
    mu_L, sigma_L = lt_param(mu, sigma, LT_ay)
    r = max(int(r), min_hizmet_r(mu_L, sigma_L, hedef))
    sim = simpy_dogrula(Q, r, mu, LT_ay, h, p, S, talep_serisi, n_rep=n_rep)
    adim = max(1, int(np.ceil(sigma_L * 0.25)))
    deneme = 0
    while sim["HZ"] < hedef and deneme < 20:
        r += adim
        sim = simpy_dogrula(Q, r, mu, LT_ay, h, p, S, talep_serisi, n_rep=n_rep)
        deneme += 1
    return r, sim
...
def parca_optimize(parca_kodu: str,
    ...
    if segment == SEGMENT_DUSUK:
        ...
        r_eoq  = min_hizmet_r(mu_L, sigma_L)
        SS_eoq = max(0, int(round(r_eoq - mu_L)))
    ...
    Q_eoq  = max(int(round(np.sqrt(2*S*max(mu_hist,1)/max(h,1e-9)))), 1)
    r_eoq  = min_hizmet_r(mu_L_h, sigma_L_h)
    SS_eoq = max(0, int(round(r_eoq - mu_L_h)))
    ...
    opt_r, sim = hizmet_duzeyi_garanti(opt["Q"], opt["r"], mu_hist, sig_hist,
                                       lt, h, p, S, tahmin_listesi, n_rep=n_rep)
    opt["r"] = opt_r
    opt["SS"] = max(0, int(round(opt_r - mu_L_t)))
